...
## run a test to create logfile with pwm & rpm stats:
command:
```
sudo ./dell_fans_test.sh
```

## open the created logfile to find the state changes.
...
## edit the dell_fans.conf file to use your settings:
...
## install: copy the files in the system-folders
commands:
```
...
if no error - enable service to start at boot:
command:
```
